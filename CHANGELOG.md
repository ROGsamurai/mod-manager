...
- Installed Mods showed "Game Root Folder" for mods that were actually installed to BepInEx/plugins (TextureReplacer and every other plugin-folder mod), so the installed row disagreed with the install row. It now reports where the files really went; mods installed by older versions are corrected automatically
